...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader, config):
        ...
        # Default data loader for FFHQ
        self.ffhq_loader = celeba_loader  # This will be the FFHQ loader if dataset is FFHQ
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD', 'FFHQ']:
    ...
    def create_labels(self, c_org, c_dim=8, dataset='FFHQ', selected_attrs=None):
        """Generate target domain labels for debugging and testing."""
        c_trg_list = []
        
        if dataset == 'FFHQ':
            # For FFHQ age dataset - create a target for each age bin
            for i in range(c_dim):
                c_trg = torch.zeros_like(c_org)
                c_trg[:, i] = 1  # Set the target age bin to 1
            ...
            # Create targets for CelebA attributes
            for i in range(c_dim):
                c_trg = c_org.clone()
            ...
            # Create targets for RaFD expressions
            for i in range(c_dim):
    ...
    def classification_loss(self, logit, target, dataset='CelebA'):
        ...
        if dataset == 'CelebA' or dataset == 'FFHQ':
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                # Save the original and translated images separately
                # First save the original image
                result_path = os.path.join(self.result_dir, '{}-original.jpg'.format(i+1))
                save_image(self.denorm(x_real.data.cpu()), result_path, nrow=1, padding=0)
                print('Saved original image into {}...'.format(result_path))
                
                # Save each age transformation as a separate image
                for age_idx, x_fake in enumerate(x_fake_list[1:]):  # Skip the first one as it's the original
                    age_label = self.selected_attrs[age_idx]
                    result_path = os.path.join(self.result_dir, '{}-age-{}.jpg'.format(i+1, age_label))
                    save_image(self.denorm(x_fake.data.cpu()), result_path, nrow=1, padding=0)
                    print('Saved age {} transformation into {}...'.format(age_label, result_path))
                
                # Also save the concatenated image for reference
                x_concat = torch.cat(x_fake_list, dim=3)
                result_path = os.path.join(self.result_dir, '{}-all-ages.jpg'.format(i+1))
                save_image(self.denorm(x_concat.data.cpu()), result_path, nrow=1, padding=0)
                print('Saved all age transformations into {}...'.format(result_path))
    ...
    def test_single_image(self, image_path, output_path):
        """Test the model on a single image and generate age progressions."""
        ...
        try:
            # Load and process image directly (no MTCNN)
            from PIL import Image
            import os
            from torchvision import transforms
            
            # Use the exact same transforms as in the data_loader
            ...
            x_real = transform(img).unsqueeze(0).to(device)
            
            # Create target domain labels for each age group
            c_trg_list = []
            for i in range(self.c_dim):
                c_trg = torch.zeros(1, self.c_dim)
                c_trg[0, i] = 1
            ...
            with torch.no_grad():
                ...
                print(f"✅ Age progression results saved to: {output_path}")
